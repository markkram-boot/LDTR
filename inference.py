...
def relation_infer(h, out, model, obj_token, rln_token, nms=False, map_=False):
    ...
    for batch_id in range(h.shape[0]):
        ...
        node_idx_dict = {}
        for ii, idx in enumerate(node_id):
            node_idx_dict[ii] = idx.cpu().numpy()  
        node_idx4attn_map.append(node_idx_dict)
...
#             print("pred_rel shape: ", pred_rel)
#             print('node_pairs: ', node_pairs_valid[pred_rel].cpu().numpy())
    ...
    if map_:
        return pred_nodes, pred_edges, node_idx4attn_map, pred_nodes_boxes, pred_nodes_boxes_score, pred_nodes_boxes_class, pred_edges_boxes_score, pred_edges_boxes_class
    else:
        return pred_nodes, pred_edges, node_idx4attn_map, 