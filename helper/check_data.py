...
#     y_ = np.int32(np.linspace(5, w-5-p_w, 32))
    x_ = np.int32(np.linspace(5, h-5-p_h, 128)) # pos: 64, neg: 8
    y_ = np.int32(np.linspace(5, w-5-p_w, 128)) # pos: 64, neg: 8
...
#             for patch, patch_seg, patch_coord, patch_edge in zip(patch_list, seg_list, patch_coord_list, patch_edge_list):
                ...
                if np.sum(patch_seg/255) < 500 or (xmax-xmin < 0.7 and ymax-ymin < 0.7):
                    cnt += 1
                    continue
                img = np.zeros((patch_size[0], patch_size[1], 3))
                for n1_idx, n2_idx in patch_edge:
                    ...
                    cv2.circle(img, (p1[0], p1[1]), 3, (0,255,0), -1)
                    cv2.circle(img, (p2[0], p2[1]), 3, (0,255,0), -1)
                    cv2.line(img, (p1[0], p1[1]), (p2[0], p2[1]), (255,255,255), 2)
#                     for x, y, _ in patch_coord:
#                         xx, yy = (x*patch_size[0]).astype('int32'), (y*patch_size[0]).astype('int32')
#                         cv2.circle(img, (xx, yy), 3, (0,255,0), -1)
                cv2.imwrite(os.path.join(save_path, '%d_seg.png'%image_id), img)
                cv2.imwrite(os.path.join(save_path, '%d_data.png'%image_id), patch)
...
#     root_dir = "/data/weiweidu/sat2graph_inputs/CA_Bray_2001_railroads"
...
#         raw_files.append(root_dir + "/ca_bray_railroads_2001_region%d" % ind)
#         seg_files.append(root_dir + "/ca_bray_railroads_2001_gt_region%d.png" % ind)
#         vtk_files.append(root_dir + "/ca_bray_railroads_2001_region%d.p" % ind)
