...
#         start = np.array((start[0],start[1],0))
        ...
        if patch_coordinates.shape[0] < 2 or patch_edge.shape[0] < 1:
            ...
            for patch, patch_seg, patch_coord, patch_edge in zip(patch_list, seg_list, mod_patch_coord_list, mod_patch_edge_list):
                xmin, xmax = min(patch_coord[:,0]), max(patch_coord[:,0])
                ymin, ymax = min(patch_coord[:,1]), max(patch_coord[:,1])
                if np.sum(patch_seg/255) < 400 or (xmax-xmin < 0.7 and ymax-ymin < 0.7) or\
                    patch.shape[0] != patch_size[0] or patch.shape[1] != patch_size[0] :
                    cnt += 1
                    continue
                if patch.shape[2] != 3:
                    continue
...
if __name__ == "__main__":
    ...
    print('Preparing Positive Train Data')
