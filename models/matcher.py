...
#         print(outputs["pred_logits"].shape, outputs["pred_logits"].flatten(0, 1).shape, outputs["pred_logits"].flatten(0, 1).softmax(-1).shape, cost_class.shape)
        # Final cost matrix
...
#                 indices.append((np.array([-1,-1]), np.array([-1,-1])))
            else:
