...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, query_embed, pos_embeds):
        ...
        # decoder
        hs, inter_references, self_attn_maps = self.decoder(
        ...
        return hs, init_reference_out, inter_references_out, self_attn_maps, None, None
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, pos, src_padding_mask=None):
        ...
        tgt2, attn_scores = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1))
...
#             tgt_rln = tgt[:,-2:-1,:]
#             tgt_con = tgt[:,-1:,:]
#             print('tgt_rln, tgt_con shape: ', tgt_obj.shape, tgt_rln.shape, tgt_con.shape)
...
#             tgt =torch.cat((tgt_obj, tgt_rln, tgt_con),1)
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, return_attn_maps=True):
        super().__init__()
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios, pos_embeds,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, attn_scores = layer(output, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, pos_embeds, src_padding_mask)
        ...
        if self.return_attn_maps:
            return output, reference_points, attention_scores_list
