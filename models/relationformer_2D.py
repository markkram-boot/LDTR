...
class RelationFormer(nn.Module):
    ...
    def __init__(self, encoder, decoder, config):
        ...
        if config.MODEL.DECODER.CONN_TOKEN > 0:
            self.connection_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM*2, config.MODEL.DECODER.HIDDEN_DIM, 2, 3)
        else:
            seld.connection_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM*2, config.MODEL.DECODER.HIDDEN_DIM, 2, 3)
    ...
    def forward(self, samples, seg=True):
        ...
        hs, init_reference, inter_references, self_attn_maps, _, _ = self.decoder(
        ...
        return hs, out, self_attn_maps, srcs, init_reference, inter_references
