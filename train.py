...
#     from losses import SetCriterion
    from losses_map import SetCriterion
...
#         config, mode='split'
#     )
    train_ds = build_road_network_data(
        config, mode='train'
    )
...
#     )
    ...
    if args.resume:
        print('*** Resume the training from %s ***'%args.resume)
...
#                                 {'total': x["loss"]["total"], 'class': x["loss"]["class"], \
#                                   'nodes': x["loss"]["nodes"], 'edges': x["loss"]["edges"],\
#                                   'conns': x["loss"]["conns"]  }) 
    # show the average loss not the latest batch in the progress bar
    RunningAverage(output_transform=lambda x: x["loss"]["total"]).attach(trainer,'total')
    RunningAverage(output_transform=lambda x: x["loss"]["class"]).attach(trainer,'class')
    RunningAverage(output_transform=lambda x: x["loss"]["nodes"]).attach(trainer,'nodes')
    RunningAverage(output_transform=lambda x: x["loss"]["edges"]).attach(trainer,'edges')
    RunningAverage(output_transform=lambda x: x["loss"]["conns"]).attach(trainer,'conns')
    pbar = ProgressBar(persist=True) 
    pbar.attach(trainer, ['total', 'class', 'nodes', 'edges', 'conns'])
